...
class WAPoliceCrimeService {
  ...
  private async getDistrictCrimeData(district: string): Promise<CrimeData | null> {
    try {
      ...
      console.warn(`Real crime data not available for ${district}, using statistical baseline:`, error)
    }

    // Use statistical baseline data (based on real WA Police patterns)
    return this.generateStatisticalBaselineData(district, undefined)
  }
  ...
  /**
   * Generate statistical baseline data based on real WA Police patterns (fallback)
   */
  private generateStatisticalBaselineData(district: string, suburbCode?: string): CrimeData {
